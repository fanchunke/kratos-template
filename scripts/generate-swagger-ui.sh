...
GEN_DIR="./third_party/OpenAPI/swagger-ui"
...
# populate openapi.yaml
tmp="    urls: ["
for i in $(find "$GEN_DIR" -name "openapi.yaml"); do
...
# recreate swagger-ui, delete all except openapi.yaml
find "$GEN_DIR" -type f -not -name "openapi.yaml" -delete
mkdir -p "$GEN_DIR"
